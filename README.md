# Redux Niveau Initiation
Redux is a predictable state container for JavaScript apps.  It helps you write applications that behave consistently, run in different environments (client, server, and native), and are easy to test. On top of that, it provides a great developer experience, such as live code editing combined with a time traveling debugger.
## Plan de la formation
![plan](images/plan.png)
## Public concerné
![public](images/public.png)
## Connaissances requises
![connaissance](images/connaissancerequise.png)
## Ressources & Outils 
![ressourceetoutils](images/ressourceetoutils.png)
## Redux Kezako ?
![redux](images/reduxkezako.png)
## Inspiration
![inspiration](images/inspiration.png)
## Avantage Redux
![avantage](images/avantage.png)
![avantage](images/avantage2.png)
![avantage](images/avantage3.png)
## Présentation & concepts clés
![conceptscles](images/conceptscles.png)
## Gestion état & cycle de vie
![cycle](images/cycledevie.png)
## Créateurs d'actions
![action](images/action.png)
## Fonction reducer
![reducer](images/reducer.png)
## Store
![store](images/store.png)
## Cycle de vie unidirectionnel
![uni](images/unidir.png)
## Flux de données
![flux](images/fludedonnees.png)
## Créer des actions
![creer](images/creeractions.png)
![type](images/typeaction.png)
![type](images/typeaction2.png)
![type](images/addaction.png)
## Définir des actions
![definiraction](images/definiraction.png)
## Dispatch des actions
![dispatch](images/dispatch.png)
## Définir la fonction reducer
![definir](images/definirreducer.png)
![initail](images/initalstate.png)
![red](images/red.png)
## Défintion du Store
![def](images/defstore.png)
## Création du store
![create store](images/createstore.png)
## Actions
![action store](images/actionstore.png)
## Optimiser le store
![optimiserlestore](images/optimiserlestore.png)
## Utiliser des middmewares
![middleware](images/midleware.png)
![an](images/enhancer.png)